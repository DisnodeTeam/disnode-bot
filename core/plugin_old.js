 ...
 */
LoadAllPlugins(guild) {
  var self = this;

  return new Promise(function(resolve, reject) {
    

    guild.instances.length = 0;
    guild.plugins.length = 0;

    Logger.Info("PluginManager-" + instance.guildID, "LoadAllPlugins", "Loading All Plugins!")

    async.waterfall([
      // Load Server Plugins
      function(cb) {
        self.GetPluginFiles("./servers/" + self.guildID, true).then(function(plugins) {
          ...
          for (var i = 0; i < plugins.length; i++) {
            ...
            instance.plugins.push(plugins[i]);
            if (plugins[i].launch) {
      ...
      function(cb) {
        ...
        self.GetPluginFiles("./plugins/", false).then(function(plugins) {
          ...
          for (var i = 0; i < plugins.length; i++) {
            ...
            for (var x = 0; x < instance.plugins.length; x++) {
