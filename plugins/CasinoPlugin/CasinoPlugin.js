    ...
    self.utils.getPlayer(command).then(function(player){
      ...
      self.disnode.bot.SendEmbed(command.msg.channel_id, {
        ...
        fields: [ {
          ...
          name: 'Disnode Ultra',
          inline: false,
          value: "**Help us keep the bots running 24/7 and get great perks by doing so by Purchasing Ultra :** https://disnodeteam.com/ultra",
        }],
          footer: {}
      });
    });
    ...
    self.utils.getPlayer(command).then(function(player) {
      switch (command.params[0]) {
        ...
        default:
          ...
          cmds += "`!casino transfer` - TAllows you to transfer your money in this bot to other bots."
          self.disnode.bot.SendEmbed(command.msg.channel_id, {
            color: 1433628,
            author: {},
            fields: [ {
  ...
  sendTransaction(player, amount, to, channel, caller){
    ...
    self.dsc.postTransactionRequest(player.id, amount, to).then(function(resp) {
      if(resp.status == 200){
        player.money -= amount
        var data = resp.data;
        caller.utils.DB.Update("players", {"id":player.id}, player);
