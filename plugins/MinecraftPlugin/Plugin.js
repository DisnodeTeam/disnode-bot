...
class MinecraftPlugin {
  ...
  default(command){ // This is the default command that is ran when you do your bot prefix and your plugin prefix example   !ping
    ...
    self.disnode.bot.SendCompactEmbed(command.msg.channelID, "Minecraft Plugin Commands", commandHelp)
  }
  commandSkin(command){
    var self = this;
    var user = command.params[0];
    if(!user){
      self.disnode.bot.SendCompactEmbed(command.msg.channelID, "Minecraft Plugin Commands", ":warning: Please enter a Minecraft user!")
    }
    ...
    axios.get("https://mcapi.ca/skin/"+user+"/250/true").then(function(res){
      if(res){
        ...
        self.disnode.bot.SendCompactEmbed(command.msg.channelID, "Minecraft Plugin Commands", ":warning: No Skin Found")
      }

    }).catch(function(err){
      self.disnode.bot.SendCompactEmbed(command.msg.channelID, "Minecraft Plugin Commands", ":warning: API Error: " + err)
    });
  }
}
